import { 
    ...
    PointerLockControls, 
    Sky, Stars, Text, 
    useCursor, useProgress } from '@react-three/drei'
...
const Scene = forwardRef((props,ref) => {
    ...
    let shiftKeyPressed = useRef(false);
    ...
    useEffect(() => {
        ...
        if (persRef.current) {
            persRef.current.addEventListener( 'unlock', function () {
                ...
                refInstruct.current.style.display = 'flex';
            })
        }
    },[persRef.current])
    ...
    const onKeyDown = function ( event ) {
        switch ( event.code ) {
            ...
            case 'ShiftLeft':    
            case 'ShiftRight':
                shiftKeyPressed.current = event.shiftKey;
                break;
    ...
    useFrame(() => {
        ...
        if (persRef.current.isLocked == true) {
            ...
            velocity.current.x -= velocity.current.x *  (shiftKeyPressed.current ? 30.0 : 15.0) * delta;
            velocity.current.z -= velocity.current.z * (shiftKeyPressed.current ? 30.0 : 15.0) * delta;
            ...
            if ( persRef.current.getObject().position.y < (shiftKeyPressed.current ? 2 : 3) ) {

                velocity.current.y = 0;
                persRef.current.getObject().position.y = (shiftKeyPressed.current ? 2 : 3);
...
const FloatingImages = () => {
    ...
    useEffect(() => {
        ...
        if (progress == 100) {
            gsap
            .timeline()
            // .to([refBlock.current,refBlock])
            .to([refBlock.current,refInstruct.current],{
                display:'flex',
            })
            .to([refBlock.current,refInstruct.current], {
                opacity:1,
